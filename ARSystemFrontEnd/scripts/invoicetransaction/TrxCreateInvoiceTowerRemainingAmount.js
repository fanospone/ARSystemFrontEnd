...
jQuery(document).ready(function () {
    ...
    $("#btReturn").unbind().click(function () {
        ...
        oTable.rows('.active').every(function (rowIdx, tableLoop, rowLoop) {
            Data.CheckedRow.push(this.data());
        });
        if (Data.CheckedRow.length == 0)
...
var Table = {
    ...
    Search: function () {
        ...
        fsInvoiceCategoryId = $('input[name=rbPercent]:checked').val();
        ...
        var params = {
            ...
            InvoiceCategoryId: fsInvoiceCategoryId,
        ...
        var tblSummaryData = $("#tblSummaryData").DataTable({
            ...
            "ajax": {
                "url": "/api/CreateInvoiceTowerRemainingAmount/grid",
                "type": "POST",
                "datatype": "json",
                "data": params,
            },
            ...
            "columns": [
                {
                    ...
                    mRender: function (data, type, full) {
                        ...
                        { strReturn += "<label style='display:none' id='" + full.trxArDetailRemainingAmountId + "' class='mt-checkbox mt-checkbox-single mt-checkbox-outline " + full.trxArDetailRemainingAmountId + "'><input type='checkbox' class='checkboxes' /><span></span></label>"; }
                        else
                        {
                            if (Helper.IsElementExistsInArray(full.trxArDetailRemainingAmountId, Data.RowSelected)) {
                                if (Helper.IsElementExistsInArray(full.trxArDetailRemainingAmountId, Data.RowSelectedSite)) {
                                    strReturn += "<label style='display:none' id='" + full.trxArDetailRemainingAmountId + "' class='mt-checkbox mt-checkbox-single mt-checkbox-outline " + full.trxArDetailRemainingAmountId + "'><input type='checkbox' class='checkboxes' /><span></span></label>";
                                }
                                else {
                                    //$("#Row" + full.trxArDetailRemainingAmountId).addClass("active");
                                    strReturn += "<label id='" + full.trxArDetailRemainingAmountId + "' class='mt-checkbox mt-checkbox-single mt-checkbox-outline " + full.trxArDetailRemainingAmountId + "'><input type='checkbox' checked class='checkboxes' /><span></span></label>";
                                }
                            }
                            else {
                                strReturn += "<label id='" + full.trxArDetailRemainingAmountId + "' class='mt-checkbox mt-checkbox-single mt-checkbox-outline " + full.trxArDetailRemainingAmountId + "'><input type='checkbox' class='checkboxes' /><span></span></label>";
                            }
                        }
                        return strReturn;
                    }
    ...
    Export: function () {
        ...
        var InvoiceCategoryId = fsInvoiceCategoryId;
        ...
        window.location.href = "/InvoiceTransaction/TrxCreateInvoiceTowerRemainingAmount/Export?strCompanyId=" + strCompanyId + "&strOperator=" + strOperator
            + "&strBapsType=" + strBapsType + "&strPeriodInvoice=" + strPeriodInvoice + "&strInvoiceType=" + strInvoiceType + "&strRegional=" + strRegional + "&InvoiceCategoryId=" + InvoiceCategoryId
            ...
            "columns": [

                {
                    ...
                    mRender: function (data, type, full) {
                        ...
                        {
                            if (Helper.IsElementExistsInArray(full.trxArDetailRemainingAmountId, Data.RowSelectedDeptHead)) {
...
var TableSite = {
    ...
    GetSelectedSiteData: function (listID) {
        ...
        $.ajax({
            url: "/api/CreateInvoiceTowerRemainingAmount/SitelistGrid",
            type: "POST",
            dataType: "json",
            async: false,
            ...
            "columns": [
                {
                    mRender: function (data, type, full) {
                        var strReturn = "";
                        strReturn += "<button type='button' title='Delete' class='btn btn-xs red btDeleteSite' id='btDeleteSite" + full.trxArDetailRemainingAmountId + "'><i class='fa fa-trash'></i></button>";
                        return strReturn;
                    }
                },
                { data: "SONumber" },
...
var Process = {
    AddToSiteList: function () {
        ...
        if (Data.RowSelected.length == 0)
            ...
            if (ValidateStatus.Result != "")
                ...
                oTable.rows('.active').every(function (rowIdx, tableLoop, rowLoop) {
                    var checkBoxId = this.data().trxArDetailRemainingAmountId;
    ...
    CreateInvoice: function () {
        ...
        var params = {
            ...
            ListTrxArDetailRemaining: Data.SiteRow,
            SumADiscount: $("#tbDiscount").val().replace(/,/g, '')
        }

        $.ajax({
            url: "/api/CreateInvoiceTowerRemainingAmount/CreateTowerInvoice",
            type: "POST",
            dataType: "json",
            contentType: "application/json",
            data: JSON.stringify(params),
    ...
    ReturnInvoice: function () {
        ...
        if (Data.CheckedRow.length == 0)
            ...
            var params = {
                ListTrxArDetailRemaining: Data.CheckedRow,
                ReturnRemarks: $('#tbRemarksCancel').val(),
            }

            $.ajax({
                url: "/api/CreateInvoiceTowerRemainingAmount/CancelTowerInvoice",
                type: "POST",
                dataType: "json",
                contentType: "application/json",
                data: JSON.stringify(params),
    ...
    ApproveCancelInvoice: function () {
        ...
        if (Data.RowSelectedDeptHead.length == 0)
            ...
            var params = {
                ListTrxArDetailRemaining: ajaxData
            }
            $.ajax({
                url: "/api/CreateInvoiceTowerRemainingAmount/ApproveCancelTowerInvoice",
                type: "POST",
                dataType: "json",
                contentType: "application/json",
                data: JSON.stringify(params),
...
var Helper = {
    ...
    ValidateSelectedRow: function () {
        var elect = 'ELECT';
        var numberOfElect = 0;
        var count = Data.CheckedRow.length;
        var isValid = false;
        $.each(Data.CheckedRow, function (index, item) {
            if (item.BapsType == elect) {
                numberOfElect++;
            }
        });
        var oTableSite = $('#tblSiteData').dataTable();
        if (numberOfElect == 0) {
            ...
            if (oTableSite.fnGetData().length > 0) {

                var firstRow = oTableSite.fnGetData(0);
                if (firstRow.BapsType != elect) {
                    isValid = true;
                }
                else {
                    Common.Alert.Warning("There is Electricity in Existing Site Data")
                    isValid = false;
                }
            }
            else
            ...
            if (oTableSite.fnGetData().length > 0) {
                ...
                if (firstRow.BapsType == elect) {
                    ...
                    Common.Alert.Warning("All BAPS Type Should be Electricity")
                    isValid = false;
                }
            }
            else {
            ...
            Common.Alert.Warning("If you select a site whose BAPS Type is Electricty, other sites also must have Electricity BAPS Type")
            isValid = false;
        }
        return isValid;
    },
    ValidateStatus: function () {
        ...
        var l = Ladda.create(document.querySelector("#btAddSite"));
        $.ajax({
            url: "/api/CreateInvoiceTowerRemainingAmount/GetValidateResult",
            type: "POST",
            dataType: "json",
            async: false,
            contentType: "application/json",
    ...
    GetListId: function () {
        ...
        var AjaxData = [];
        $.ajax({
            url: "/api/CreateInvoiceTowerRemainingAmount/GetListId",
            type: "POST",
            dataType: "json",
            async: false,
            contentType: "application/json",
