...
var Table = {
    Summary: function () {
        ...
        var tblSummary = $("#tblSummaryMonthlys").DataTable({
            ...
            "ajax": {
                "url": "/api/CashInRemarkQuarterly/grid",
                "type": "POST",
                "datatype": "json",
                "data": params
            },
            buttons: [
                { text: 'Excel', className: 'btn green btn-outline', action: function (e, dt, node, config) { Table.ExportSummary('tblSummaryMonthlys', 'Quarterly (Q' + $("#slSearchQuarter").val() + ')', 'ARR Monitoring Cash In (Quarterly - Q' + $("#slSearchQuarter").val()+')'); } },
...
var Helper = {
    GetQuarterStaff: function () {
        ...
        var total = "TOTAL Q" + q + " " + $("#slSearchYear").val() //TOTAL Q1 2021
        $("#th1").html(total);
        $("#th2").html(total);
        $("#thVariance").html("VARIANCE Q" + q + " " + $("#slSearchYear").val());
